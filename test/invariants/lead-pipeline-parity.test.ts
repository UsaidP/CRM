...
describe('Lead Pipeline Parity Ratchet', () => {
  it('prisma.lead.create is only called inside src/lib/domain/lead-creation.ts', () => {
    ...
    for (const filePath of allFiles) {
      if (filePath.endsWith('src/lib/domain/lead-creation.ts')) {
        continue;
      }

      const content = readFileSync(filePath, 'utf-8');
      if (content.includes('prisma.lead.create')) {
        const lines = content.split('\n');
        lines.forEach((line, index) => {
          if (line.includes('prisma.lead.create')) {
            violations.push({
